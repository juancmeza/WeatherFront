...
export const LocationCard = React.memo(function TutorCard({data, updateSelectedCity, removeFromSavedLocations}) {
    ...
    <Row p={1.5} gap={1} bgcolor={'linear-gradient(#ffb514, #ff8503)'} borderRadius={16} className={styles.row}>
      ...
      <Item mr={1} position={'right'}>
        ...
        <IconButton className={styles.action} classes={iconBtnStyles}>
          <DeleteForeverOutlinedIcon onClick={() => removeFromSavedLocations(data.location)}/>
        </IconButton>
      </Item>
    </Row>
  );
